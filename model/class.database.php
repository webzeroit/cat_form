...
class MySQL
{
	...
	private $db_user    = "mtfsthsu_cform";          	// user name
	private $db_pass    = "mtfsthsu_cform";		// password
	private $db_dbname  = "mtfsthsu_cform";         	// database name
	...
	private $db_user    = "dev";          	// user name
	private $db_pass    = "pa55w0rd";		// password
	private $db_dbname  = "cform_db";         	// database name
	private $db_charset = "utf8";         		// optional character set (i.e. utf8)
	private $db_pcon    = false;      		// use persistent connection?
	...
	const SQLVALUE_DATE_IT  = "date-it";	
	...
	static public function SQLValue($value, $datatype = self::SQLVALUE_TEXT) {
		...
		switch (strtolower(trim($datatype))) {
			...
			case "date":
				...
				break;
			case "date-it":
				if (self::IsDate($value ,'DD/MM/YYYY')) {
					$return_value = "STR_TO_DATE('" . $value . "','%d/%m/%Y' )";
				} else {
					$return_value = "NULL";
				}
				break;		
